...
{
  local lib_ui is lexicon(
    "Console", uiConsole@,
    "Banner", uiBanner@,
    "Warning", uiWarning@,
    "Error", uiError@,
    "ShowPorts", uiShowPorts@,
    "Fatal", uiFatal@,
    "AssertAccel", uiAssertAccel@,
    "Debug", uiDebug@,
    "DebugNode", uiDebugNode@,
    "DebugAxes", uiDebugAxes@
  ).

  local ui_announce is 0.
  local ui_announceMsg is "".

  local ui_debug     is true.  // Debug messages on console and screen
  local ui_debugNode is true. // Explain node planning
  local ui_debugAxes is false. // Explain 3-axis navigation e.g. docking

  local logconsole   is true. //Save console to log.txt / 0:/<CRAFT NAME>.txt

  local ui_DebugStb is vecdraw(v(0,0,0), v(0,0,0), GREEN, "Stb", 1, false).
  local ui_DebugUp is vecdraw(v(0,0,0), v(0,0,0), BLUE, "Up", 1, false).
  local ui_DebugFwd is vecdraw(v(0,0,0), v(0,0,0), RED, "Fwd", 1, false).

  local ui_myPort is vecdraw(v(0,0,0), v(0,0,0), YELLOW, "Ship", 1, false).
  local ui_hisPort is vecdraw(v(0,0,0), v(0,0,0), PURPLE, "Dock", 1, false).
