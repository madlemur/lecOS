...
local ui is import("ramp/lib_ui.ks").
local util is import("ramp/lib_util.ks").
...
FUNCTION TimeToLong {
    ...
    LOCAL TgtLong IS util["LongitudeTo360"](lng).
    LOCAL ShipLong is util["LongitudeTo360"](SHIP:LONGITUDE).
...
// MAIN PROGRAM
// ************
loadscript("ramp/node.ks"). wait 0.

if ship:status = "ORBITING" {
    // Zero the orbit inclination
    IF abs(OBT:INCLINATION) > 0.1 {
        ui["Banner"]("Deorbit","Setting an equatorial orbit").
        loadscript("ramp/node_inc_equ.ks").  wait 0.
        RUNPATH("ramp/node_inc_equ.ks",0).  wait 0.
        delscript("ramp/node_inc_equ.ks").  wait 0.
        RUNPATH("ramp/node.ks").
    }
    // Circularize the orbit
    if obt:eccentricity > 0.01 {
        ui["Banner"]("Deorbit","Circularizing the orbit").
        loadscript("ramp/circ.ks"). wait 0.
        runpath("ramp/circ.ks"). wait 0.
        delscript("ramp/circ.ks"). wait 0.
    }
    ...
    IF Land_Lat >= 91 AND Land_Lng >= 361 { // Means no paramenter was given.
        ...
        IF ValidTarget {
            SET Land_Lat to util["ReduceTo360"](TARGET:GEOPOSITION:LAT()).
            SET Land_Lng TO util["LongitudeTo360"](TARGET:GEOPOSITION:LNG()).
        }
        ELSE {
            SET Land_Lat TO 0.
            SET Land_Lng TO util["LongitudeTo360"](SHIP:GEOPOSITION:LNG()).
        }
    }
    ...
    Set Deorbit_Long to util["LongitudeTo360"](Land_Lng - 180).
    Set PlaneChangeLong to util["LongitudeTo360"](Land_Lng - 270).
    ...
    if TotIncDV > 1 { // Only burn if it matters.
        ui["Banner"]("Deorbit","Burning dV of " + round(TotIncDV,1) + " m/s @ anti-normal to change plane.").
    ...
    ui["Banner"]("Deorbit","Burning dV of " + round(Deorbit_dV,1) + " m/s retrograde to deorbit.").
    LOCAL nd IS NODE(time:seconds + TimeToLong(Deorbit_Long+phi) , 0, 0, Deorbit_dV).
    add nd.
    runpath("ramp/node.ks").
    ui["Banner"]("Deorbit","Deorbit burn done").

    // Warp about over to landing site
    loadscript("ramp/warp.ks"). wait 0.
    run warp(eta:periapsis - 120). wait 0.
    delscript("ramp/warp.ks"). wait 2.

    // Brake the ship in prepartion to land
    SET BreakingDeltaV to VELOCITYAT(ship,eta:periapsis):orbit:mag.
    ui["Banner"]("Deorbit","Burning dV of " + round(BreakingDeltaV,1) + " m/s retrograde to brake ship.").
    SET ND TO NODE(time:seconds + eta:periapsis , 0, 0, -BreakingDeltaV).
    add nd.
    runpath("ramp/node.ks").
    ui["Banner"]("Deorbit","Brake burn done").

}
ELSE IF SHIP:STATUS = "SUB_ORBITAL" {
    LOCK LandingSite TO SHIP:GEOPOSITION.
}
delscript("ramp/node.ks").
// Try to land
