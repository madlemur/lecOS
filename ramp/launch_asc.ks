...
local lParts is import("ramp/lib_parts.ks").
local ui is import("ramp/lib_ui.ks").
local util is import("ramp/lib_util.ks").

ui["Banner"]("Launch","Launching to an orbit of " + round(apo/1000) + "km and heading of " + hdglaunch + "º").
...
function ascentStaging {
  ...
  if Nsrb > 0 {
    ...
    ui["Banner"]("Launch","Stage " + ThisStage + " separated. " + Nsrb + " SRBs discarded.").
  } else if (Nlfo > 0) {
    wait until stage:ready.
    stage.
    set launch_tStage to time:seconds.
    ui["Banner"]("Launch","Stage " + ThisStage + " separated. " + Nlfo + " Engines out.").
  } else if Neng = 0 {
    wait until stage:ready.
    stage.
    set launch_tStage to time:seconds.
    ui["Banner"]("Launch","Stage " + ThisStage + " activated").
  }
...
function ascentFairing {
  if ship:altitude > ship:body:atm:height {
    lParts["DeployFairings"]().
    ui["Banner"]("Launch","Discard fairings").
  }
}
...
wait until util["IsShipFacing"](heading(hdglaunch,0):vector).
...
if stage:resourceslex:haskey("LiquidFuel") {
  if stage:resourceslex["LiquidFuel"]:capacity > 0 { // Checks to avoid NaN error
    if stage:resourceslex["LiquidFuel"]:amount / stage:resourceslex["LiquidFuel"]:capacity < 0.1 {
      stage.
      ui["Banner"]("Launch","Discarding tank").
      wait until stage:ready.
    }
  }
  ...
  until ship:availablethrust > 0 {
    stage.
    ui["Banner"]("Launch","Discard non-propulsive stage").
    wait until stage:ready.
  }
}
